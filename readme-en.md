...
A robotic arm project workspace containing mechanical models and 3D printing resources, an STM32F103 firmware project, D-H and URDF reference files, and a LaTeX project report.
...
| `Latex Report/` | LaTeX report source, bibliography, and existing PDF | [main.tex](<Latex Report/main.tex>), [reference.bib](<Latex Report/reference.bib>), [main.pdf](<Latex Report/main.pdf>) |
...
Edit the report in `Latex Report/main.tex` and maintain the bibliography in `reference.bib`. An existing `main.pdf` is present, but it has not been recompiled or checked against the current source during this documentation work.
...
- **Not yet verified:** Firmware builds and hardware operation, ROS 2 builds and visualization, kinematic parameters, and LaTeX compilation. This documentation work involved only document and static file checks.
- **Version control:** The current [.gitignore](.gitignore) excludes the entire `Latex Report/` directory, so local report changes are not automatically included in this repository's version history.

## README Maintenance

Whenever an AI assistant modifies project files, it must update this README and the [Chinese version](readme.md) before completing the task. Updates must cover the relevant content, last updated date, and change log. Both versions must remain aligned, without requiring another reminder. The full rules are maintained in [AGENTS.md](AGENTS.md).
